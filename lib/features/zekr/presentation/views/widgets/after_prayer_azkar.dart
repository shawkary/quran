...
class AzkarAfterPrayer extends StatelessWidget {
  const AzkarAfterPrayer({super.key, required this.azkarModel});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: customAppBar(
          context: context, text1: 'أذكار بعد الصلاة', centerTitle: true),
      body: ListView.separated(
        ...
        itemBuilder: (context, index) => Column(
          ...
          children: [
            Text(
              azkarModel.prayerLaterAzkar![index].text!,
            ...
            Row(
              ...
              children: [
                if (azkarModel.prayerLaterAzkar![index].count! > 4)
                  ElevatedButton(
                      onPressed: () => navigateTo(context, const Counter()),
                      child: const Text('عداد الذكر')),
                const Spacer(),
                Text(
                  '( ${azkarModel.prayerLaterAzkar![index].count} )',
        ...
        itemCount: azkarModel.prayerLaterAzkar!.length,
      ),
    );
  }
}
