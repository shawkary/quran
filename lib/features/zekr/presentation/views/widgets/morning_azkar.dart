...
class MorningAzkar extends StatelessWidget {
  const MorningAzkar({super.key, required this.azkarModel});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: customAppBar(
          context: context, text1: 'أذكار الصباح', centerTitle: true),
      body: ListView.separated(
        ...
        itemBuilder: (context, index) => Column(
          ...
          children: [
            Text(
              azkarModel.morningAzkar![index].text!,
            ...
            Row(
              ...
              children: [
                if (azkarModel.morningAzkar![index].count! > 4)
                  ElevatedButton(
                      onPressed: () => navigateTo(context, const Counter()),
                      child: const Text('عداد الذكر')),
                const Spacer(),
                Text(
                  '( ${azkarModel.morningAzkar![index].count} )',
        ...
        itemCount: azkarModel.morningAzkar!.length,
      ),
    );
  }
}
